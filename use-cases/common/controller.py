"""Reusable client for use-case controllers talking to the orchestrator.

Controllers (the apps that drive human-in-the-loop / multi-stage flows) keep
repeating the same glue: submit a workflow, poll its task statuses, and publish
a logical graph + live state to the orchestrator's solution view. This module
wraps that so a controller only writes its graph and its decision logic.

Pairs with the services' `common.sequential` / `common.concurrent` helpers.

Usage:
    from common.controller import OrchestratorClient, inline, decode

    oc = OrchestratorClient()                      # reads ORCHESTRATOR_URL / key from env
    oc.publish_graph("my-view", "My Pipeline", nodes, edges)
    wf = oc.submit(blueprint, dockerinfo, [inline({"factor": 1.0})])
    status, tasks = oc.workflow(wf)                # tasks: {node_key: task_dict}
    statuses = oc.task_statuses(wf)                # {node_key: "running"|"completed"|...}
    out = oc.task_output(wf, "summarizer")         # decoded inline output of a node
    oc.publish_state("my-view", {...}, stage="awaiting", message="...", detail=...)
"""
